...
## Q 도커를 사용하는 이유는 무엇일까
**1. 개발환경 세팅이 편하다.**
일반적으로 개발자들은 자신의 로컬 컴퓨터에서 개발을 하고 그 결과물을 서버에서 실행시켜 배포한다. 이렇게 되려면 로컬의 개발환경과 서버의 개발환경이 같아야 한다.
또한 새로운 서비스를 만드는데 이 서비스가 다른 환경에서 개발되었다면 또 설정을 새로 해주어야 한다.이는 굉장히 불편하며 메뉴얼도 없고 전임자도 없을시 골치 아픔
도커는 이러한 문제를 해결하는데 도움을 준다. 개발환경 문제로 에러가 났을 때 다른 환경에서도 쉽게 똑같은 환경을 구축할 수 있도록 도와준다.

**2. 수많은 컨테이너를 효율적으로 관리하기 위해**
어떤 서비스가 점차 확장되어 많은 서버와 리소스를 가지고 운영을 해야 하는데 물리적인 하드웨어(장비)와 자주 발생하는 서비스 배포의 관계가 1,2개가 아니라 수천, 수만, 때론 수백만 서버와 배포를 관리하기 위해서는 기존의 방식으로는 감당 되지 않음 구글의 경우에도 많은 서비스와 인프라를 컨테이너로 구축하여 사용한다.
...
## Q. 어떤 경우에 Azure를 사용하면 좋을까?

**자유로운 개발환경**
Visual Studio가 Azure와 통합이 잘 되어 있음 개발자 들이 Azure 포털 사이트나 커맨드 라인에서 작업하지 않아도 Visual Studio에서 개발이 가능하다.

**안전하고 신뢰할 수 있는 클라우드 환경**
> 보안을 위하여 매년 1조 정도의 예산을 사용할 정도로 보안에 대하여 투자를 많이함 강력한 보안 및 컴플라이언스 기능을 제공하여 데이터 및 응용 프로그램을 안전하게 보호 Azure Active Directory를 통한 강력한 인증 및 권한 부여, 데이터 암호화, 네트워크 보안 그리고 감사 및 모니터링 도구를 통해 신뢰성 있는 클라우드 환경을 제공 규정 준수를 충족시키기 위한 다양한 인증서를 보유하고 있어 금융, 건강, 제조 등의 업계에서도 신뢰
...
![image](https://github.com/team-imad-study/study-question/assets/16646208/7e0288aa-21f1-40d1-b496-d7ff08803411)
