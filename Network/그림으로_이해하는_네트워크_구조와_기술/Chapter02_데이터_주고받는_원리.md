...
- 우리가 흔히 접하는 장치로 2, 3계층의 특성을 모두 가지고 있다.(라우팅을 할 수있다.)
	...
	- 2계층:  전송된 데이터를 해당 네트워크 매체로 어떻게 전송할지 결정하고, 데이터를 프레임으로 나누고 프레임에 MAC 주소 등의 정보를 추가하는 역할을 한다
...
- EGP는 다른 AS를 가진 네트워크와의 통신을 하기 위한 프로토콜
...
- 자동 라우팅: 현재의 라우팅 프로토콜들은 수동 설정이나 관리가
