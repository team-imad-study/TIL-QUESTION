...
## NTP서버에는 계층화된 Stratum서버가 있는데 클라이언트는 가장 낮은 Stratum의 서버와 통신하여 시간을 얻는다고 하는데 가장 상위 Stratum 서버에서 가장 낮은 Stratum과의 통신하는 동안 시간의 간격이 있을 텐데 우리에게 어떻게 클라이언트에게 정확한 시간을 알려줄 수 있는 것일까? 
