...
1. 발신 : 메일 발신자 이메일 전송 MAIL
2. 서버 : 250코드와 응답확인
3. 발신 : 메일 수신자 이메일 전송 RCPT
4. 서버 : 250코드와 응답 확인
5. 발신 : 데이터 요청을 보냄(본문 작성을 위함) DATA
6. 서버 : 354 코드로 메일 본문 작성 응답 확인
7. 발신 : 본문 작성 후 마지막에 ‘.’특수문자를 보내면 본문 전송이 끝나고 Quit 명령어를 전송함
8. 서버 : 완료 응답하고 TCP연결 세션이 끊킴
