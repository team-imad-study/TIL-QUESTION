...
## Q. DDR SDRAM 은 버젼별로 네이밍이 붙습니다 ex) DDR1, DDR2, DDR3, DDR4, DDR5 이러한 버젼별로 다른네이밍을 가진 램들을 한컴퓨터에서 혼용해서 사용할수 있을까요?

![r2](https://github.com/team-imad-study/study-question/assets/136051281/b65d7ce0-234c-48d1-8f74-e91ba884d55b)

정답은 X 입니다! 이렇게 DDR의 버젼별로 대역폭과 소비전력량의
차이로 인해 서로다른 핀패키지와 핀갯수를 가지기 때문에
이러한 차이를 인지하고 메인보드와 커넥터를 사용하고자 
하는 램의 지원사양과 맞추어야 합니다
ex) DDR5 = 288핀 BGA 패키지
    DDR4 = 288핀 DIMM 패키지
...
## Q 물리 주소와 논리 주소는 왜 나누어져 있는 것일까? 간단하게 물리 주소만 사용하면 안 되는가?

만약 물리 주소만을 이용하고 싶다면, CPU는 시시각각 변하는 모든 물리 주소에 저장된 모든 내용을 전부 다 알고 있어야 한다.
물리 주소 내에 몇 번지에 뭐가 저장되어있는지... CPU의 입장에서 매번 이걸 기억하기는 쉽지 않은 일이다. CPU의 저장 공간이라고 해봤자 저장 공간이 작은 레지스터일 뿐이기도 하며 그렇다고 매번 메모리에 접근하여 확인하는 것은 메모리 접근 시간이 늘어나 굉장히 비효율적일 것이다. 때문에 그냥 0번지부터 시작한다고 간주하고 생각을 하자 라는 논리 주소를 이용하는 것이다.
CPU 입장에서는 0번지부터 시작하는 논리 주소만을 해석하고 그 사이에 논리 주소 - 물리 주소를 변환해주는 장치를 하나 더 두는 것이다.
